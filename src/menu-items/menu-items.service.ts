...
export class MenuItemsService {
  ...
  */
  ...
  getChildren(parent: any, menuItems: MenuItem[]): any {
    return menuItems
      .map((menuItem) => {
        if (parent.id === menuItem.parentId) {
          return {
            ...menuItem,
            children: this.getChildren(menuItem, menuItems),
          };
        }
      })
      .filter(Boolean);
  ...
  async getMenuItems() {
    const menuItems: MenuItem[] = await this.menuItemRepository
